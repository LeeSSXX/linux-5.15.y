...
bool amlogic_is_valid(struct amlogic_table *apt)
{
	...
	if (strncmp(apt->header.version, APT_VERSION_STRING,
		    strlen(APT_VERSION_STRING))) {
...
bool amlogic_should_parse_block(struct parsed_partitions *state)
{
	...
	if (!apt_blkdevs) {
		pr_debug(
			"Amlogic partition: apt_blkdevs is not set, no block device should be parsed\n");
		return false;
	}
	if (!apt_blkdevs[0]) {
		pr_debug(
			"Amlogic partition: apt_blkdevs is empty, no block devices should be parsed\n");
		return false;
	}
	if (!strncmp(apt_blkdevs, "all", 3)) {
		pr_debug(
			"Amlogic partition: apt_blkdevs set to all, all block devices should be parsed\n");
		return true;
	}
	...
	for (char *c = apt_blkdevs;; ++c) {
		...
		case ',':
		case '\0':
			if (strncmp(blkdev, state->disk->disk_name,
				    c - blkdev)) {
				if (*c == '\0') {
					return false;
				}
...
int amlogic_partition(struct parsed_partitions *state)
{
	...
	pr_debug(
		"Amlogic partition: partition table is valid, now parsing it\n");

	for (u32 j = 0;
	     j < le32_to_cpu(apt->header.count) && j < state->limit - 1; j++) {
		struct amlogic_partition *part = apt->parts + j;
		...
		put_partition(state, j + 1, offset, size);

		info = &state->parts[j + 1].info;
		name_min =
			min_t(size_t, sizeof info->volname, sizeof part->name);
